...
import { Layers, Plus, Edit2, Search, Filter, BookOpen } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { ACCOUNT_TYPES, ACCOUNT_SUBTYPES, type AccountType } from "@/lib/accounting/coa.model";
          ...
          <div className="flex gap-2 w-full md:w-auto">
            ...
            <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
              <DialogTrigger asChild>
                <Button size="sm" className="gap-2 h-9 text-xs" disabled={!selectedInstId} onClick={() => setEditingAcc(null)}>
                  ...
                  <div className="grid gap-4 py-4 text-xs">
                    <div className="grid grid-cols-3 gap-4">
                      <div className="grid gap-2">
                        <Label>Code</Label>
                        <Input name="code" defaultValue={editingAcc?.code} placeholder="1000" required />
                      </div>
                      <div className="grid gap-2 col-span-2">
                        <Label>Account Name</Label>
                        <Input name="name" defaultValue={editingAcc?.name} placeholder="Petty Cash" required />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="grid gap-2">
                        <Label>Subtype</Label>
                        <Input name="subtype" defaultValue={editingAcc?.subtype} placeholder="Current Asset" />
                      </div>
                    </div>
                  </div>
                  <DialogFooter>
            ...
            <Card className="border-none ring-1 ring-border shadow-xl bg-card">
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {isLoading ? (
                      <TableRow><TableCell colSpan={5} className="text-center py-12 text-xs">Synchronizing ledger...</TableCell></TableRow>
                    ) : filteredAccounts.length === 0 ? (
                      <TableRow><TableCell colSpan={5} className="text-center py-12 text-xs text-muted-foreground">No accounts registered in this category.</TableCell></TableRow>
