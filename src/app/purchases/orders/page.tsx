...
import { ClipboardCheck, Plus, Search, Filter, History, MoreVertical, Loader2, PackageCheck, Zap } from "lucide-react";
...
export default function PurchaseOrdersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      ...
      items: [], // Scaffolding for full line items
      status: 'Draft',
    };

    try {
      await createPurchaseOrder(db, selectedInstId, data, user.uid);
      toast({ title: "Purchase Order Raised" });
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Procurement Pipeline Hub</p>
            </div>
          </div>
        ...
        {!selectedInstId ? (
          ...
          <Card className="border-none ring-1 ring-border shadow-xl bg-card overflow-hidden">
            <CardHeader className="py-3 px-6 border-b border-border/50 bg-secondary/10 flex flex-row items-center justify-between">
              ...
              <Badge variant="outline" className="text-[10px] font-bold uppercase tracking-widest">Pipeline Value: REAL-TIME</Badge>
            </CardHeader>
          ...
          <DialogContent className="max-w-md">
            <form onSubmit={handleSubmit}>
              <DialogHeader>
                ...
                <CardDescription className="text-xs uppercase font-black tracking-tight">Institutional Procurement Hub</CardDescription>
              </DialogHeader>
              ...
              <div className="grid gap-4 py-4 text-xs">
                <div className="space-y-2">
                  ...
                  <Select name="supplierId" required>
                    <SelectTrigger className="h-10"><SelectValue placeholder="Search Vendor Registry..." /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  <Label>Estimated Total (Tax Inc)</Label>
                ...
                <div className="p-3 bg-primary/5 border border-primary/10 rounded-lg flex gap-3 items-start">
                  ...
                  <p className="text-[10px] text-muted-foreground leading-relaxed italic">
                    <strong>Automation King:</strong> Generating this PO will assign a unique institutional identifier. Line items can be added during the Draft phase.
                  </p>
                </div>
              </div>
              ...
              <DialogFooter>
                <Button type="submit" disabled={isProcessing} className="h-10 font-bold uppercase text-xs w-full shadow-lg shadow-primary/20">
                  {isProcessing ? <Loader2 className="size-3 animate-spin mr-2" /> : <ClipboardCheck className="size-3 mr-2" />} Commit Draft PO
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
